...
REST API for managing investment assets, portfolios, and accounting reports. Provides CRUD operations for assets, banks, asset types and sub-types, and integrates with external quotation services.

## Features

- **Asset management** — Full CRUD for investment assets (quantity, average price, quotation, value, category, bank, type).
- **Master data** — CRUD for Banks, Asset Types (`AssetType`) and Asset Sub-Types (`AssetSubType`).
...
| Banks                    | `/asset/bank`                    | GET `/` (list), GET `/{id}`, GET `/query?name=NAME`, POST `/`, DELETE `/{id}` |
...
Below are concise descriptions and example cURL commands for the main endpoints that were added/updated. The examples assume the API is served at `http://localhost:8080`.

1) Banks (Master data)

- Base path: `http://localhost:8080/asset/bank`
- Model: `Bank` — fields: `id`, `name`
...
- Find by name
  ```bash
  curl -X GET "http://localhost:8080/asset/bank/query?name=MyBank"
  ```
...
Create example (relationships are sent as nested objects with `id`):
...
├── controller/                        # REST controllers (Asset, Bank, AssetType, AssetSubType, AssetReport)
├── excpetion/                         # AssetNotFoundException, HandlerError, AssetErrorDetails
├── external/                          # Quotation API client (QuotationExternal, DTOs)
├── model/                             # JPA entities (Asset, Bank, AssetType, AssetSubType, AccountingReportAsset)
├── repository/                        # JPA repositories (interfaces + *RepositoryImpl)
├── service/                           # Business logic (asset, report, masters)
└── utils/                             # Utilities
```

Layered flow: **Controller → Service → Repository Impl → JPA Repository (interface)**. Master data (Bank, AssetType, AssetSubType) follow the same pattern.
